...
from api.api_client import (
    ShakaSportsApiClient,
)  # Импортируем класс для взаимодействия с API
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    text=buttons, callback_data=f"ac_{key}"
                )  # Создаем кнопку для каждой академии
            ]
        )
...
async def create_age_division_keyboard(state: FSMContext):
    ...
    async with ShakaSportsApiClient(event_id) as client:
        ...
        )  # Получаем возрастные категории из API
        age_divs_info = get_division_info(
            age_divisions
        )  # Получаем информацию о возрастных категориях

    keyboard = []
    if event_id:
        for index, (key, buttons) in enumerate(age_divs_info.items()):
            keyboard.append(
                [InlineKeyboardButton(text=buttons, callback_data=f"age_{index}")]
...
async def create_weight_keyboard(state: FSMContext):
    ...
    async with ShakaSportsApiClient(event_id) as client:
        ...
        age_id = list(age_divisions.keys())[
            age_id_index
        ]  # Получаем id возрастной категории
        sex_info = has_empty_dict(
            age_divisions[age_id]["sex_divisions"]
        )  # Получаем данные о поле
        division_info = has_empty_dict(
            age_divisions[age_id]["belt_divisions"]
        )  # Получаем данные о поясах
        weight = has_empty_dict(
            age_divisions[age_id]["weights"]
        )  # Получаем данные о весах

        await state.update_data(
            sex_info=sex_info, division_info=division_info, weight_info=weight
    ...
    if event_id:
        for age_id_index, (keys, buttons) in enumerate(weight_info.items()):
            keyboard.append(
                [
                    InlineKeyboardButton(
                        text=f"{str(buttons)} кг", callback_data=f"w_{age_id_index}"
                    )
                ]
            )  # Создаем кнопки для каждой весовой категории
...
async def create_sex_div_keyboard(state: FSMContext):
    ...
    if sex_info:
        for index, (keys, buttons) in enumerate(sex_info.items()):
            keyboard.append(
                [InlineKeyboardButton(text=buttons, callback_data=f"sex_{index}")]
...
async def create_division_keyboard(state: FSMContext):
    ...
    """
    data = await state.get_data()  # Получаем данные из FSM
    division_info = data.get(
        "division_info", []
    )  # Получаем информацию об уровне подготовки

    keyboard = []
    if division_info:
        for index, (keys, buttons) in enumerate(division_info.items()):
            keyboard.append(
                [InlineKeyboardButton(text=buttons, callback_data=f"div_{index}")]
            )  # Создаем кнопки для каждого уровня подготовки
...
def create_confirm_registration_keyboard():
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="Показать", callback_data="confirm_reg"),
                # Кнопка "Показать"
                create_back_step_button(),
            ]
        ]
    )  # Кнопка "Назад"
...
def create_finish_registration_keyboard():
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="Зарегистрировать", callback_data="finish_reg"
                ),
...
def create_back_step_button():
    ...
    return InlineKeyboardButton(
        text=back_step_btn, callback_data=back_step_btn
...
def create_start_btn_button():
    ...
    """
    return InlineKeyboardButton(
        text=start_btn, callback_data=start_btn
    )  # Возвращаем кнопку "Сначала"
...
def create_back_step_keyboard():
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text=back_step_btn, callback_data=back_step_btn)]
        ]
...
def create_start_btn_keyboard():
    ...
    """
    return InlineKeyboardMarkup(
        inline_keyboard=[[create_start_btn_button()]]
    )  # Возвращаем клавиатуру с кнопкой "Сначала"
    ...
    if event_id:
        return InlineKeyboardMarkup(
            inline_keyboard=[
                ...
                [
                    InlineKeyboardButton(
                        text=f"Зарегистрировать еще на Турнир №{event_id}",
...
def create_some_error_button():
    ...
    """
    return InlineKeyboardButton(
        text=key_error_text, callback_data=start_btn
    )  # Возвращаем кнопку "key_error_text"
